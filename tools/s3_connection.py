...
@tool("upload_to_s3_tool", description="Uploads a file to S3 and returns the public URL")
...
def upload_to_s3(file_path, s3_key):
    ...
    log(f"Uploading {file_path} to S3 with key: {s3_key}, bucket: {S3_APPAREL_BUCKET_NAME}")
    
    try:
        s3.upload_file(file_path, S3_APPAREL_BUCKET_NAME, s3_key, ExtraArgs={"ContentType": "image/png"})
